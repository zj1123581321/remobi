...
UNIT="$ROOT/systemd/remobi-debug.service"
fail() { printf 'FAIL: %s\n' "$1" >&2; exit 1; }
has() { grep -F -- "$2" "$1" >/dev/null || fail "$3"; }

has "$UNIT" 'WorkingDirectory=/home/zlx/projects/oss/remobi' 'debug path must be canonical main repo'
has "$UNIT" 'ExecStart=/home/zlx/.local/share/fnm/aliases/default/bin/pnpm exec tsx cli.ts serve --host 127.0.0.1 --port 7691 --base-path /remobi --config /home/zlx/projects/oss/remobi/.omo/remobi-debug.config.ts -- herdr session attach remobi-dev' 'debug command contract changed'
