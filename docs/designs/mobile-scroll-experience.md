...
**2026-08-21 实测结论**：herdr 每个 SGR 滚轮事件只滚 1 行；默认值已从 3 改为 1，
手指位移与内容位移恢复 1:1（card remobi-20260821-07）。
...
### 实测后的第二轮（2026-08-21 当天）

真机实测暴露两个新问题，都已定位并修复：

**一、手指位移与内容位移呈 3:1**
`linesPerWheel` 默认值 3 来自 tmux 惯例，而 **herdr 每个 SGR 滚轮事件实际只滚 1 行**。
默认值已改为 1（PR #27）。这个 fork 专攻 herdr，默认值应按 herdr 的真实行为设定。

**二、松手后惯性阶段一卡一卡**
实测 herdr 的滚轮吞吐（`spikes/wheel-latency/`，三次独立运行一致）：

| 指标 | 实测值 |
|---|---|
| 单次滚轮响应延迟 p50 | 3 ms（**延迟不是瓶颈**） |
| 有效重绘上限 | 56–58 Hz |
| **1:1 映射边界** | **40 Hz** |
| 120 Hz 发送时的合并率 | 52%（360 次发送只产生 169–174 次输出） |

rAF 每帧发送在 60Hz 屏上即 60 次/秒，正落在饱和区——请求被 herdr 的渲染循环合并，
画面更新间隔变得不规则，这就是「一卡一卡」。

修法是把**发送**节流到 30 Hz（`sendIntervalMs: 33`，PR #29），相对 40 Hz 边界留 25% 余量。
与最初那个 24ms 限流的本质区别：**节流只推迟发送时机，不丢弃任何位移**。
这条不变式由一条双向反证的测试锁死——去掉节流会红，改成丢弃位移也会红。

**附带修掉的真 bug**：WebKit 会在 snapshot 应用完成前派发 `pageshow`，此时 socket 已 OPEN
但 `lastProvenFreshAt` 仍为 0，旧判据判为「不新鲜」遂建立第二条 WebSocket——
手机息屏唤醒即可触发（PR #30）。原以为只是 CI flaky，查下去是生产缺陷。

### 当前状态

| 症状 | 状态 |
|---|---|
| 手指滑很长、内容只动一点 | 已修（限流丢位移 + `linesPerWheel` 校准） |
| 滑动卡顿 | 已修（消息风暴、热路径布局、手势争抢） |
| 惯性阶段一卡一卡 | 已修（发送节流到 30 Hz） |
| 不跟手（网络往返延迟） | **仍在**，当前架构下无解，见增量 2 的证伪 |

**真机验收（2026-08-21）**：用户在 iPhone 上实测确认**手感可接受**，滑动这条主线收工。
验收路径是 Tailscale → debug 实例（`/remobi` → 127.0.0.1:7691，连 herdr `remobi-dev` 会话）；
prod 实例（7681，连 herdr `default`）随后也已重启到同一提交，两端注入配置一致
（`sendIntervalMs=33 / linesPerWheel=1 / maxLinesPerSend=24`，旧字段 `wheelIntervalMs`
与 `sensitivity` 已消失）。

这是本设计唯一一条来自**用户真实入口层**的证据——库函数绿、单测绿、CI 绿都不能替代它。
在此之前「已修」只是本机测量与反证支撑的推断。

`sendIntervalMs` 是配置项，可按真机手感调：仍有顿挫往 50（20 Hz）调，
发涩不跟手往 25（40 Hz）调。
...
**2026-08-21 实测结论**：herdr 每个 SGR 滚轮事件只滚 1 行；`linesPerWheel` 默认值
已从 3 改为 1（card remobi-20260821-07）。
