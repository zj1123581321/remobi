...
### 增量 2：服务端 scrollback 影子历史（spike 先行）
...
| 增量 | 内容 | 依赖 | 并行 |
|---|---|---|---|
| 1 | 跟手滚动引擎 | 无 | 与 2-spike 并行 |
| 2-spike | 帧差分重建取证 | 无 | 与 1 并行 |
| 2 | 影子历史 + 协议 | 2-spike GO | 串行 |
| 3 | 客户端原生滚动层 | 1 + 2 | 串行 |

增量 1 与 2-spike 文件不重叠（客户端手势层 vs 服务端会话层），横切并行。
